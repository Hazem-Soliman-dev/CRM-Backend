...
const paymentValidation = [
  ...
  body('payment_method').isIn(['Cash', 'Credit Card', 'Bank Transfer', 'Check', 'Other']).withMessage('Invalid payment method'),
  body('payment_status').optional().isIn(['Pending', 'Completed', 'Failed', 'Refunded', 'Partially Refunded']).withMessage('Invalid payment status'),
...
const updatePaymentValidation = [
  ...
  body('payment_method').optional().isIn(['Cash', 'Credit Card', 'Bank Transfer', 'Check', 'Other']).withMessage('Invalid payment method'),
  body('payment_status').optional().isIn(['Pending', 'Completed', 'Failed', 'Refunded', 'Partially Refunded']).withMessage('Invalid payment status'),
  body('transaction_id').optional().isString().withMessage('Transaction ID must be a string'),
...
router.get('/', authorize('admin', 'manager', 'agent'), getAllPayments);
...
router.get('/:id', authorize('admin', 'manager', 'agent'), getPaymentById);

// POST /api/v1/payments - Create new payment
router.post('/', authorize('admin', 'manager', 'agent'), paymentValidation, validate, createPayment);

// PUT /api/v1/payments/:id - Update payment
router.put('/:id', authorize('admin', 'manager', 'agent'), updatePaymentValidation, validate, updatePayment);
...
router.patch('/:id/status', authorize('admin', 'manager', 'agent'),
  body('payment_status').isIn(['Pending', 'Completed', 'Failed', 'Refunded', 'Partially Refunded']).withMessage('Invalid payment status'),
  validate, updatePaymentStatus);

// POST /api/v1/payments/:id/refund - Process refund
router.post('/:id/refund', authorize('admin', 'manager'),
...
router.get('/customer/:customerId', authorize('admin', 'manager', 'agent'), getCustomerPayments);

// GET /api/v1/payments/booking/:bookingId - Get payments by booking
router.get('/booking/:bookingId', authorize('admin', 'manager', 'agent'), getPaymentsByBooking);
